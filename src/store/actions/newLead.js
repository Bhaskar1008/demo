...
export const createLead = (formData) => {
  return async (dispatch) => {
    ...
    if (result.errMsg.length > 0) {
      return dispatch(createLeadSuccess(result.errMsg));
    }
  };
};
...
export const editLead = (formData, id) => {
  return async (dispatch) => {
    ...
    return dispatch(editLeadSuccess(result.errMsg));
  };
};
...
export const fetchLeadDetails = (id) => {
  return async (dispatch) => {
    ...
    if (result.errMsg.length > 0) {
      ...
      return dispatch(
        fetchLeadDetailsSuccess({ ...result.errMsg[0], appointmentDetails: null })
      );
    }
    let leadArr = [];
