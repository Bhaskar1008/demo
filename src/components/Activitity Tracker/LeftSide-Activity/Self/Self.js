...
const Self = () => {
  ...
  const api = async (upcomingdate,upcoming)=>{
    ...
    if (data.errCode === -1) {
      setLoader(false)
      setCurrentPastData(data.errMsg);
      setPastDataContainer(data.errMsg);

       setDataContainer(data.errMsg)
       setCurrentPastDataLn(data?.errMsg?.length)
    } else {
  ...
  const currentpastapi = async (pastdate)=>{  
    ...
    if (data.errCode === -1) {
      setLoader(false)
      setPastData(data.errMsg);
       setPastDataLn(data?.errMsg?.length)
    } else {
      setLoader(false)
    }
