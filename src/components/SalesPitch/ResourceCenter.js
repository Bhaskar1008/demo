...
const ResourceCenter = () => {
  ...
  useEffect(() => {
    axiosRequest
      ...
      .then((res) => {
        console.log("newLead.", res?.errMsg?.[0]?._id);
        if (res.length !== 0) {
          console.log("head", res);
          setProductData(res?.errMsg);
          setActiveId(res?.errMsg?.[0]?._id);
          setActiveTabName(res.errMsg?.[0]?.ResourceCenterName);
        }
      })
      .catch();
