...
const SalesPitch = () => {
  ...
  async function handleTestimonilClick() {
    ...
    for (let i in initialresult?.errMsg) {
      if (initialresult?.errMsg[i].channelCode.includes(currUserChannelCode)) {
        if (initialresult?.errMsg[i].status == 1) {
          if (initialresult?.errMsg[i].filePath.length != 0) {
            result.push(initialresult?.errMsg[i]);
          }
        }
      }
    }
  ...
  async function handlePresentationClick() {
    ...
    for (let i in initialresult.errMsg) {
      if (initialresult.errMsg[i].channelCode.includes(currUserChannelCode)) {
        let activeImage = [];
        for (let j of initialresult.errMsg[i].filePath) {
          if (j.active == "1") {
            activeImage.push(j);
          }
        }
        initialresult.errMsg[i].filePath = activeImage;
        if (initialresult.errMsg[i].status == "1") {
  ...
  async function handleProductClick() {
    ...
    const result = [];
    for (let curr of initialresult.errMsg[0]) {
