...
const DailyBussiness = () => {
  ...
  const showModal = async () => {
    ...
    try {
      ...
      setTodayGoalCreated(res.errMsg);
    } catch (error) {
      // console.log(error);
    }
...
const onDateRangeChange = async (dates, dateStrings) => {
  ...
  const GetGraph = async (id) => {
    try {
      ...
      if (res.errMsg.graph_data) {
        let responseArray = [...res.errMsg.graph_data];
      ...
      try {
        todayGoalCreated?._id
          ...
          console.log("API CALL",APIcall)
            if(updateAPIcall){
              if(updateAPIcall?.errCode === -1){
                message.success("Goal updated successfully");
              }
            }else if (APIcall){
              if(APIcall?.errCode === -1){
  ...
  const exportButtonGet = async () => {
    ...
    if(resExportButton?.errCode === -1){
      downloadFile(resExportButton?.errMsg, 'DailybusinessCommitment')
