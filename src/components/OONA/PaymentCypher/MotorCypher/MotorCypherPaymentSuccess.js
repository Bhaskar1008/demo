  ...
  const handleGetRequest = async () => {
    try {
      ...
      .then((response) => {
        ...
        setGetPolicyData(response?.data?.errMsg)
...
;
  const GetPaymentmethods = async () => {
    ...
    const axios = require('axios');
      ...
      axios.request(config)
      .then((response) => {
        console.log("response?.errCode", response);
        if (response?.data?.errCode === -1) {
          if (response?.status === 200) {
            if (response?.data?.errMsg?.data?.length === 1) {
              console.log("sindledata");
              let eventdata = {
                paymentText:response?.data?.errMsg?.data[0]?.paymentText,
            ...
            setloader(false);
            let paymnetmode = response?.data?.errMsg;
      ...
      .then((response) => {
        ...
        if(event.paymentText === 'CC' || event.paymentText === 'GC' || event.paymentText === "Card / E-wallet"){
          ...
          if (response?.data?.errCode === -1) {
            ...
            let URLdata = response?.data?.errMsg?.data?.body?.redirectUrl;
            // let Valuedata = response?.data?.errMsg?.data?.value;
            // return
            if (response?.status === 200) {
          ...
          if(event.paymentText === "Payment Link"){
            if(response?.errCode === -1){
              message.success(response?.errMsg?.message);
            }else{
              setloader(false);
              console.log("Error");
            } 
