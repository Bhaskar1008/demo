...
import MessageEmail from "../../../OONA/SharableLink/components/messageEmail";
...
export const DrawerFunction = ({ onClose, showDrawer,isfromprofile}) => {
  ...
  useEffect(() => {
    ...
    dispatch(actions.fetchAllCommercialStructure(url1, result =>{
      if (result.errCode === -1) {
        const initialArr = result?.errMsg?.lov
        initialArr.forEach(v => {v.isActive = false;});
        setCommercialStructure(initialArr)
        console.log("data==========>>>",result?.errMsg?.lov)
        if(result?.errMsg?.lov?.length == 1){
          setLoader(true);
          let comercial = result?.errMsg?.lov[0]
...
//           setLoader(false);
//           const documentId = response?.errMsg?.documentId;
