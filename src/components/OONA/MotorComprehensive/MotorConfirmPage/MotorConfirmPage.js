...
const MotorConfirmPage = () => {
  ...
  let coveragesApi = useSelector(
    (state) => state?.motorCoverage?.generateCoverages?.errMsg?.coverages
  );
  ...
  const leadId = useSelector(
    (state) => state?.motorQuotation?.formalQuotationSucess?.errMsg?.leadId
  );
  ...
  const motorQuote = useSelector(
    (state) => state?.motorQuotation?.motorQutotionSucess?.errMsg
  );
  ...
  //     // if (result?.errMsg?.oonaPolicyResponse?.technicalControls.length > 0) {

  //     if (result?.errMsg?.oonaQuotationResponse?.technicalControls.length > 0) {
  //       if (
  //         result?.errMsg?.oonaQuotationResponse?.technicalControls?.[0]
  //           ?.code === 1624 ||
  //         result?.errMsg?.oonaQuotationResponse?.technicalControls?.[0]
  ...
  //     } else if (
  //       result?.errMsg?.oonaPolicyResponse?.paymentBreakdown == null ||
  //       result?.errMsg?.oonaPolicyResponse?.technicalControls.length > 0
  //     ) {
  ...
  const onchangetoNext = async () => {
    ...
    if (result?.errCode === -1) {
      ...
      if (result?.errMsg?.oonaPolicyResponse?.technicalControls.length > 0) {
        if (result?.errMsg?.oonaPolicyResponse?.technicalControls?.[0]?.code === 1624 ||
          result?.errMsg?.oonaPolicyResponse?.technicalControls?.[0]?.error === "Payment is required") {
          history.push("/motor-payment-pay-now");
        ...
        result?.errMsg?.oonaQuotationResponse?.paymentBreakdown == null ||
        result?.errMsg?.oonaQuotationResponse?.technicalControls.length > 0
      ) {
        history.push("/formal-technical-control");
  ...
  const getSubAgentName = async () => {
    ...
    if (quatationData?.subAgents?.length > 0) {
      ...
      if (res.errCode === -1) {
        const selectedSubAgent = res?.errMsg?.filter(
          (item) => item.documentCode == subAgentId
        );
        console.log("selectedSubAgent: ", selectedSubAgent);
        if (res?.errMsg && Array.isArray(res.errMsg && selectedSubAgent)) {
  ...
  const getAgentName = async () => {
    if (quatationData?.agentCode?.length > 0) {
      ...
      const res = await axiosRequest.get(url);
      if (res.errCode === -1) {
        const selectedAgent = res?.errMsg?.filter(
          (item) => item.codAgt == selectedAgentcodenew
        );
        console.log("selectedSAgent: ", selectedAgent);
        if (res?.errMsg && Array.isArray(res.errMsg && selectedAgent)) {
  ...
  const getPolicyTerm = async () => {
    ...
    const res = await axiosRequest.get(url);
    if (res.errCode === -1) {
      const selectePolicyTerm = res?.errMsg?.lov?.filter(
        (item) => item.COD_FRACC_PAGO == selectedPolicyterm
      );
      console.log("selectedSAgent: ", selectePolicyTerm);
      if (res?.errMsg && Array.isArray(res.errMsg && selectePolicyTerm)) {
  ...
  const getCommericalStrucure = async () => {
    ...
    if (res.errCode === -1) {
      ...
      if (res?.errMsg?.data) {
        selectedCommercialStructure = res?.errMsg?.data?.filter(
          (item) => item.branchCode == selectedCommercial
        );
      } else {
        selectedCommercialStructure = res?.errMsg?.lov?.filter(
      ...
      if (
        res?.errMsg &&
        Array.isArray(res.errMsg && selectedCommercialStructure)
      ) {
        if (channelCodeforUse == "CH1") {
