...
import MotorPolicyPage from "../../OONA/MotorComprehensive/MotorPolicyPage/motorlandingpage";
const MotorComprehensive = (props) => {
  ...
  const onchangetoNext = async () => {
    if (activeButton == false) {
      ...
      if(!result?.errMsg || result?.errCode !== -1){
      ...
      // console.log(
      //   "hey123",
      //   result?.errMsg[0][0]?.motor_comprehensive?.policyHolder?.documentCode
      // );
      let allQuotaioMotor = {
        errCode: -1,
        errMsg: result.errMsg[0][0],
      };

      dispatch(actions.motorFormalQuotationForm(allQuotaioMotor));
      saveDataToRedux(allQuotaioMotor?.errMsg);
      const documentId = allQuotaioMotor?.errMsg?.documentId;
      dispatch({ data: documentId, type: "CURRENT_DOCUMENTS_ID" });
  
  // return;
      if (result?.errCode === -1) {
        if (
          result?.errMsg[0][0].oonaQuotationResponse.technicalControls.length >
          0
        ) {
          history.push("/motor-technical-control");
          setLoader(false);
        } else if (
          !result?.errMsg[0][0].QC_status === "Not Required" &&
          !result?.errMsg[0][0].QC_status === "Approved"
        ) {
          ...
          if (
            result?.errMsg[0][0]?.motor_comprehensive?.policyHolder
              ?.documentCode
          ) {
