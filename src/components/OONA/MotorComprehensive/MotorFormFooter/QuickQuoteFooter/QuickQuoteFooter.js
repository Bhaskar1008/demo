...
export default function QuickQuoteFooter() {
  ...
  const FinalValue = useSelector(
    (state) => state?.motorQuotation?.motorQutotionSucess?.errMsg?.oonaQuotationResponse?.paymentBreakdown?.grossPrem
  )
  // const FinalValueformal = useSelector(
  //   (state) => state?.motorQuotation?.motorQutotionSucess?.errMsg?.oonaQuotationResponse?.grossPrem
  // )
  const FinalValueformal = useSelector(
    (state) => state?.motorQuotation?.formalQuotationSucess?.errMsg?.oonaQuotationResponse?.grossPrem
  )
