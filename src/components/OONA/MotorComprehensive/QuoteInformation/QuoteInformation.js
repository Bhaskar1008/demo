...
const QuoteInformation = () => {
  ...
  const quotationNumber = useSelector(
    (state) =>
      state?.motorQuotation?.motorQutotionSucess?.errMsg?.oonaQuotationResponse
        ?.quotationNumber
  );
  ...
  const successCoverageData = useSelector(
    (state) =>
      state?.motorQuotation?.motorQutotionSucess?.errMsg?.motor_comprehensive
        ?.coverages
  );
  ...
  const onchangetoNext = async () => {
    ...
    if (result?.errCode === -1) {
      ...
      if (result?.errMsg?.oonaQuotationResponse?.technicalControls.length > 0) {
        ...
        setopenloader(false);
      } else if (result?.errMsg?.QC_status === "Required") {
    ...
    // }
      ...
      // console.log("Im in Colseole of Else", result?.errMsg?.message);
      if (result.length !== 0) {
        // console.log("Im in Colseole of Else hh", result);
        message.error(result?.errMsg?.message);
      }
      setopenloader(false);
    }
  };
  ...
  const getAgentName = async () => {
    ...
    if (quatationData?.agentCode?.length > 0) {
      ...
      if (res.errCode === -1) {
        ...
        const selectedAgent = res?.errMsg?.filter((item) => item.codAgt == selectedAgentcodenew)
