...
export default function ConfirmPage() {
  ...
  // const result = useSelector((state) => state?.ctplpolicy?.formData?.errCode);
  ...
  //const qoutationFormDataerrMsg = useSelector(state => (state?.ctplqoutation?.formData?.errMsg))
  // const qoutationFormData = useSelector(
  //   (state) => state?.ctplqoutation?.formData?.errMsg
  // );
  const getleadId = useSelector((state) => state?.ctplqoutation?.formData);
  console.log("final-result", getleadId);
  let LeadId = getleadId?.errMsg?.leadId;
  ...
  const confrimPageData = async () => {
    ...
    //  if (result?.errCode === 4) {
    //   message.warning(result?.errMsg.commonMessage)
    // }
    ...
    //   if (result?.errMsg?.ctplResponse?.allowPayment === false) {
    //     history.push("/technical-control")
    //     // success when done -- payment
    //     // failed when done -- payment
    //   } else if (result?.errMsg?.ctplResponse?.referenceNumber === null) {
    //     history.push("/technical-control")
    //   } else {
