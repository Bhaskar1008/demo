...
import { DrawerFunction } from "../../../OONA/MotorComprehensive/MotorPolicyPage/MotorPolicyPage"
export default function QuoteCode() {
  ...
  const QuoteNext = async () => {
    if (value === "no") {
      ...
      if (result.errCode === -1 && result?.errMsg[0][0]?.oonaQuotationResponse) {
        if (result?.errMsg[0][0].oonaQuotationResponse.technicalControls.length > 0) {
          history.push('/travel-quotation-failed')
          return;
        }
        dispatch(
          setQuotationDetails({
            ...result?.errMsg[0][0]?.oonaQuotationResponse,
            quotationId: result?.errMsg[0][0]?.quotationId,
          })
        );
        saveDataToRedux(result?.errMsg[0][0]);
        history.push("/travel/success");
        // } else if (result?.errCode !== -1 && result?.errMsg[0][0]?.message) {
        //   message.error(result?.errMsg[0][0]?.message);
      } else {
        // message.error(result?.errMsg[0][0]?.message);
  ...
  const getProductList = async () => {
    ...
    if (res.errCode === -1) {
      if (res?.errMsg?.data) {
        setPolicyTypeList(res.errMsg.data);
      }
    } else {
