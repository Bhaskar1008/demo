...
export default function CTPLSuccessQuotation() {
  ...
  const documentIdListing = useSelector(
    (state) => state?.ctplqoutation?.formData.errMsg?.documentId
  ); 
  ...
  // FROM LISTING PAGE -----
  const quatationDataFromListing = useSelector((state) => state?.ctplqoutation?.formData.errMsg);
  ...
  const convertToPolicy = async () => {
    ...
    if (result?.errCode === -1) {
      ...
      if (result?.errMsg?.oonaPolicyResponse?.allowPayment === true) {
  ...
  async function downloadQuotePdf() {
    ...
    try {
      const res = await axiosRequest.post('user/printQuote', formData); // Replace with your API endpoint
          if (res.errCode === -1) {
               var a = document.createElement('a');
                a.href = res?.errMsg?.fileUrl;
                  document.body.appendChild(a);
                  a.click();
                  document.body.removeChild(a);
                setopenloader(false)
                message.success(res?.errMsg?.message)
              }else{
                setopenloader(false)
              }
