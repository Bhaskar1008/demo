...
export default function FormalQuoteInfo() {
  ...
  const proceedPolicy = async () => {
    ...
    if (res.errCode === -1) {
      setShowScreenLoader(false);
      if (res?.errMsg.oonaPolicyResponse?.technicalControls &&
        res?.errMsg.oonaPolicyResponse?.technicalControls?.length > 0) {
        const paymentRequiredError = res.errMsg.oonaPolicyResponse.technicalControls.find(
      ...
      dispatch(setPolicyGeneratedInfo(res.errMsg));
      dispatch(
        setQuotationDetails({
          ...res.errMsg.oonaQuotationResponse,
          quotationId: res.errMsg.quotationId,
          documentUploaded:res?.errMsg?.documentUploaded,
        })
      );
      history.push("/travel-policy-sucess");
