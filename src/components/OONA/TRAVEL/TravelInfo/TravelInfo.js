...
export default function TravelInfo() {
  ...
  const getTravelList = async () => {
    ...
    if (res.errCode === -1) {
      if (res?.errMsg?.data && Array.isArray(res.errMsg.data)) {
        setTravelList(res.errMsg.data);
      } else {
        message.error(res?.errMsg.data.message ?? "Something went Wrong");
      }
    }
    setShowScreenLoader(false);
  };
  ...
  const prepopulateData = async (data) => {
    if (tripInfo) {
      ...
      if (data?.currency && data?.travelPack) {
        ...
        if (res.errCode === -1) {
          setShowScreenLoader(false)
          if (res?.errMsg?.lov && Array.isArray(res?.errMsg?.lov)) {
            const options = res.errMsg.lov.map((e) => {
  ...
  const getCountryList = async (val) => {
    ...
    if (res.errCode === -1) {
      if (
        res?.errMsg?.data &&
        Array.isArray(res.errMsg.data) &&
        res.errMsg.data.length > 0
      ) {
        if (val === "D") {
          setTravelFormVal("country", [res.errMsg.data[0].NOM_PAIS]);
          let string = res.errMsg.data[0].COD_VERNACULO.toLowerCase();
        ...
        setCountryList((prev) => {
          if (prev) {
            return { ...prev, [val]: res.errMsg.data };
          } else {
            return { [val]: res.errMsg.data };
          }
        });
  ...
  const getPackageApi = async (val, _destination) => {
    ...
    if (res.errCode === -1) {
      if (res?.errMsg?.data) {
        getCurrencyFn(_destination, res.errMsg.data.packageCode);
        return res.errMsg.data;
      }
    }
    setShowScreenLoader(false);
    ...
    if (res.errCode === -1) {
      if (res?.errMsg?.lov && Array.isArray(res?.errMsg?.lov)) {
        setCurrencyList((prev) => {
          prev[_dest] = res.errMsg.lov.map((e) => ({
            label: e.COD_MON_ISO,
            value: e.COD_MON,
          }));
          return { ...prev };
        ...
        setTravelFormVal("currency", res.errMsg.lov[0].COD_MON);
        setCurrencyPersist((prev) => {
          prev[_dest] = res.errMsg.lov[0].COD_MON;
          return { ...prev };
        });
        getSumInsuredFn(res.errMsg.lov[0].COD_MON, _package.toUpperCase());
      }
    }
    setShowScreenLoader(false);
  };
  ...
  const getTripDuration = async () => {
    ...
    if (startDate && endDate) {
      ...
      if (res.errCode === -1) {
        if (res?.errMsg?.data) {
          setTravelFormVal("tripDuration", res?.errMsg?.data?.tripDuration);
          if (
            selectedDestination == "I" &&
            res?.errMsg?.data?.tripDuration <= 180
          ) {
            setTravelFormVal("oneTrip", true);
          } else if (
            selectedDestination == "D" &&
            res?.errMsg?.data?.tripDuration <= 90
          ) {
            setTravelFormVal("oneTrip", true);
          } else {
  ...
  const getOneDayTrip = async () => {
    ...
    if (travelForm.startDate && travelForm.endDate) {
      ...
      if (res.errCode === -1) {
        if (res?.errMsg?.data) {
          // setTravelFormVal("oneTrip", res.errMsg.data.oneTripOnly);
        }
      }
    }
    setShowScreenLoader(false);
  ...
  const getProductCode = async (
    ...
    if (data.errCode === -1) {
      return data?.errMsg?.data?.productCode;
    } else {
      return null;
    }
  };
