...
export default function PolicySuccessMotor() {
  ...
  const quotationDetails = motorData?.motorPolicyResponse.errMsg?.oonaPolicyResponse?.paymentBreakdown;
  const OnlyOnnaResponse = motorData?.motorPolicyResponse.errMsg?.oonaPolicyResponse
  ...
  let MotorPolicy = motorData?.motorPolicyResponse.errMsg
    ...
    section2: {
      title: (
        <div style={{ fontWeight: 500, fontSize: "17px" }}>
          Payment Reference #{motorData?.motorPolicyResponse?.errMsg?.oonaPolicyResponse?.referenceNumber}
        </div>
      ),
      data: data,
  ...
  const GetPaymentmethods = async () => {
    ...
    if (Response?.errCode === -1) {
      if (Response?.errMsg?.status === 200) {
        if (Response?.errMsg?.data?.length === 1) {
          console.log("sindledata");
          let eventdata = {
            paymentText: Response?.errMsg?.data[0]?.paymentText,
        ...
        setloader(false)
        let paymnetmode = Response?.errMsg?.data
        setPaymentTypye(paymnetmode)
        // console.log("Data=====>>>",paymnetmode)
      } else {
        message.error(Response?.errMsg?.data?.error)
        setloader(false)
      }
    } else {
      setloader(false)
  ...
  const handleclick = async (event) => {
    ...
    if (event.paymentText === 'CC' || event.paymentText === 'GC' || event.paymentText === 'Card / E-wallet') {
      if (Paymentresponse?.errCode === -1) {
        setPopupOpen(false);
        console.log("Paymentresponse", Paymentresponse);

        message.success("Please Wait Redirecting to Payment Gateway.....");
        setloader(true);
        let URLdata = Paymentresponse?.errMsg?.data?.body?.redirectUrl;
        let Valuedata = Paymentresponse?.errMsg?.data?.value;
        // return
        if (Paymentresponse?.errMsg?.data?.status === 200) {
      ...
      if (event.paymentText === "Payment Link") {
        if (Paymentresponse?.errCode === -1) {
          message.success(Paymentresponse?.errMsg?.message);
          history.push({
