...
export default function PaymentSuccessMotor() {
  ...
  const PolicyNumber = useSelector(
    (state) =>
      state?.motorQuotation?.motorPolicyResponse?.errMsg?.oonaPolicyResponse
        ?.policyNumber
  );
  ...
  const agentCodeFromPage = useSelector(
    (state) => state?.motorQuotation?.motorQutotionSucess?.errMsg?.agentCode
  );
  const agentCodeFromPageNew = useSelector(
    (state) => state?.motorQuotation?.motorPolicyResponse?.errMsg?.agentCode
  );
