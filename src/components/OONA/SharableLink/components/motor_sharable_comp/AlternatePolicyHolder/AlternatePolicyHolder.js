...
export default function AlternatePolicyHolder() {
  ...
  useEffect(() => {
    const getIdType = async () => {
      ...
      if (res.errCode === -1) {
        if (res?.errMsg?.lovOptions && Array.isArray(res.errMsg.lovOptions)) {
          setIdList(
            res.errMsg.lovOptions.map((e) => ({
              label: e.NOM_VALOR,
              value: e.ADQC_ID,
            }))
          );
  ...
  const checkVerified = (details, section) => {
    ...
    axiosRequest
      ...
      .then((response) => {
        ...
        setShowScreenLoader(false);
        if (response?.errMsg?.customerDetail) {
          const { firstName, lastName, birthday, gender } =
            response?.errMsg?.customerDetail;
        ...
        if (section == "alternate-policy" && response?.errCode !== -1) {
          setAlternatePolicyValidated("no");
        }
        if (section == "owner" && response?.errCode !== -1) {
          setOwnerValidated("no");
        }
        if (section == "driver" && response?.errCode !== -1) {
          setDriverValidated("no");
        }
        if (section == "assignee" && response?.errCode !== -1) {
          setAssigneeValidated("no");
        }
        if (section == "lessor" && response?.errCode !== -1) {
          setLessorValidated("no");
        }
        if (section == "lessee" && response?.errCode !== -1) {
