...
const PolicyGroup = () => {
  ...
  const getSubAgentList = async (agentCode) => {
    ...
    if (res.errCode === -1) {
      if (res?.errMsg && Array.isArray(res.errMsg)) {
        setOptionList((prev) => {
          prev["subAgent"] = res.errMsg.map((e) => ({
            label: e.name,
  ...
  const result = useSelector(
    (state) => state?.ctplqoutation?.formData?.errCode
  );
  ...
  const getGroupPolicy = async (agentCode) => {
    ...
    setLoader(false);
    if (res?.errCode == -1) {
      const data = res?.errMsg?.lov?.map((item) => ({
        ...item,
  ...
  const getContactList = async (policyGroupId, agentCode) => {
    ...
    setLoader(false);
    if (res?.errCode == -1) {
      const data = res?.errMsg?.lov?.map((CONTRACT) => {
  ...
  const getSubContractList = async (contactCode, agentCode) => {
    ...
    setLoader(false);
    if (res?.errCode == -1) {
      const data = res?.errMsg?.lov?.map((SUBCONTRACT) => {
