...
export default function CTPLSuccessQuotation() {
  ...
  const AgentCodeFromSelect = useSelector((state) => state?.ctplPolicy?.formData?.errMsg?.agentCode);
  console.log("AgentCodeFromSelect---", AgentCodeFromSelect);

  const qoutationFormData = useSelector((state) => state?.ctplPolicy?.formData?.errMsg
  );
