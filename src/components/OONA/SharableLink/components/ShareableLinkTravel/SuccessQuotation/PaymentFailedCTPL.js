...
export default function PaymentFailedCTPL(props) {
  ...
  const policyFormData = useSelector(state => (state?.ctplPolicy?.formData?.errMsg))
  ...
  const paymentFailed = async (PolicyNumber) => {
    ...
    try {
      ...
      if (result?.errCode === -1) {
        setloader(false)
        console.log("result----", result.errMsg[0]);
        //setPerparticulerQuotation(result.errMsg[0])
        onchangetoNext(result.errMsg[0][0])
      } else {
        setloader(false)
      }
      /// dispatch(actions.fetchLeadDetailsSuccess(result.errMsg[0]));
