...
export default function CustomerDetail() {
  ...
  const onClickConfirm = async () => {
    ...
    setShowScreenLoader(false);
    if (res.errCode === -1 && res?.errMsg?.oonaQuotationResponse) {
      let storeAllRes = res?.errMsg
      if (res.errMsg.oonaQuotationResponse.technicalControls.length > 0) {
        dispatch(
          setQuotationDetails({
            storeAllRes,
          })
      ...
      // dispatch(setQuotationDetails({...res.errMsg.oonaQuotationResponse, documentId, quotationId : res.errMsg.quotationId}))
      dispatch(
        setQuotationDetails({
          storeAllRes,
          ...res.errMsg.oonaQuotationResponse,
          quotationId: res.errMsg.quotationId,
        })
      );
      history.push("/customer/travel/success");
    } else if (res?.errCode !== -1 && res?.errMsg?.message) {
      message.error(res?.errMsg?.message);
    } else {
      message.error(res?.errMsg?.message);
  ...
  const getSubAgentName = async () => {
    if(tripData?.travelPolicyData?.subAgents?.length > 0){
      ...
      if (res.errCode === -1) {
        const selectedSubAgent = res?.errMsg?.filter((item) => item.documentCode == subAgentId)
