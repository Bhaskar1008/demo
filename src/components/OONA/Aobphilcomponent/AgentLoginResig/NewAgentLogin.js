  ...
  const AccountFormSubmit = async () => {
    ...
    try {
      ...
      if (response?.data?.errCode === -1) {
        setOpenloader(false);
        dispatch(actions.storeAgentONBoardLisenceForm(response?.data?.errMsg));
        let licenNaffilai = response?.data?.errMsg?.agentData[0].applicantType
        dispatch(actions.StoreLiAff(licenNaffilai))
        const isSubmmited = response?.data?.errMsg?.agentData[0]?.isResubmittingForm
        const aobStatus = response?.data?.errMsg.agentData[0].adminAction_OnAppliction;
        const QCshouldEmpty = response?.data?.errMsg?.agentData[0]?.QC_Discrepancy;
  ...
  const submitOtp = async () => {
    ...
    try {
      ...
      if (response?.data?.errCode === -1) {
        ...
        openConfirmPopUp();
        setIsModalOpen();
      }else if (response?.data?.errCode === 121 || response?.data?.errCode === 144) {
        setOpenloader(false)
    ...
    message.error("Please enter valid phone Number");
      ...
      try {
        ...
        if (response?.data?.errCode === -1) {
          ...
          dispatch(actions.storeAgentONBoardForm(response?.data?.errMsg));
          showModal();
        }else{
          setOpenloader(false)
        }
