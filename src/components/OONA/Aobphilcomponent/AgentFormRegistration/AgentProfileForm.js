...
const AgentProfileForm = ({messageData, setClose}) => {
  ...
  const agentCodelistingAgain = useSelector((state) => state?.cipherStore?.SaveCipherTEXT?.formData?.errMsg?.agentCode);
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        if (response?.data?.errCode === -1) {
          setLoader(false);
          console.log("gfdfghjk", response?.data?.errMsg?.data);
          if (response?.data?.errMsg?.data !== undefined) {
            setSalesOfficeData(response?.data?.errMsg?.data)
          }else{
            setLoader(false);
            setSalesOfficeData(response?.data?.errMsg?.lov)
          }
        }
      } else {
        setLoader(false);
