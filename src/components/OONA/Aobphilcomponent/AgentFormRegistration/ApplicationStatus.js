  ...
  const AccountFormSubmit = async () =>{
    ...
    try {
      ...
      if (response?.data?.errCode === -1) {
        setLoader(false)
        console.log("storeAgentAllApplicationForm", response?.data?.errMsg);
        dispatch(actions.storeAgentONBoardLisenceForm(response?.data?.errMsg));
        setAllResponseStore(response?.data?.errMsg)
        let isReSummite = response?.data?.errMsg?.agentData[0].isResubmittingForm
        if (isReSummite === true) {
          setAllQCDiscrapencyData(response?.data?.errMsg?.agentData[0]?.latest_RaisedQC)
        }else{
          setAllQCDiscrapencyData(response?.data?.errMsg?.agentData[0]?.QC_Discrepancy)
        }
        
        setApplicationID(response?.data?.errMsg?.agentData[0]?.applicationId)
      }else{
        setLoader(false)
      }
    } catch (error) {
