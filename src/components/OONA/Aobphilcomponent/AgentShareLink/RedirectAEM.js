...
const AgentShareLink = () => {
    ...
    useEffect(() => {
        ...
        dispatch(actions.fetchAllCommercialStructure(url1, result =>{
          if (result.errCode === -1) {
            setLoader(false);
            const initialArr = result?.errMsg?.lov[0]?.COD_NIVEL3
    ...
    async function callApiWithJwt(eventdata) {
        ...
        dispatch(
            actions.fetchSaveCipherText(formData, (result) => {
                ...
                if (result.errCode === -1) {
                    setLoader(false)
                    let allResponse = result?.errMsg
                    let currentURL = result?.errMsg?.responseData;
    ...
    const handleNavigation = async (event) =>{
      ...
      try {
        ...
        if (res.errCode === -1) {
          ...
          console.log("res===============>", res);
          // setTokenData() 
          let token = res?.errMsg?.encryptedData
