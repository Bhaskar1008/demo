...
const AgentBrokerLogin = () => {
        ...
        if (res.status === 200) {
          ...
          try {
            if (res.data.errCode === -1) {
              ...
              setIsModalOtp(true);
              setTimerstart("60");
              setUserObject(res.data.errMsg);
              console.log("USER ID====>>", res.data.errMsg[0][0]._id);
              setUserId(res.data.errMsg[0][0]._id);
              childRef.current.childFunction();
              setOpenloader(false);
              setLoginData(res.data.errMsg);
              // submitOtp(res.data.errMsg);
              // }
  ...
  return (
    <>
      <LoginHeader style={{ position: "absolute", top: "-35%" }} />
              ...
              >
                  ...
                  onFinish={
                    isOtpSend
                      ? () => {
                          ...
                          console.log("Verify OTP debug", otp);
                        }
                      : (values) => {
                ...
                >
                  {!isOtpSend ? (
                    ...
                    >
                      <Input size="large" placeholder="92345 67890" />
                    </Form.Item>
                  ) : (
                    <>
                      ...
                      <Text type="secondary" style={{ marginTop: 16, display: "flex", justifyContent: "space-between" }}>
                        ...
                        <span style={{ color: "#223C86", cursor: timer > 0 ? "not-allowed" : "pointer", fontWeight: "600" }} onClick={timer === 0 ? handleOtpsend : undefined}>
                          RESEND in {timer > 0 ? formatTime(timer) : "now"}
                        </span>
                      </Text>
                    </>
                  )}
